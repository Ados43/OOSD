...
public class HelloController {
    ...
    protected void onPlayTetris() {
        ...
        // Ensure background music respects config, but only starts inside Tetris
        ...
        game.setOnBack(() -> {
            mediaManager.stopBgMusic(); // stop music when exiting game
            goBackToMenu(stage);
        });
    ...
    protected void onConfiguration() {
        ...
        Label fieldWidthLabel = new Label("Field Width (cells):");
        ...
        Label fieldHeightLabel = new Label("Field Height (cells):");
        ...
        // --- Toggles (non-sound options only) ---
        ...
        grid.add(aiPlay, 0, 3);
        grid.add(aiValue, 2, 3);

        grid.add(extendedMode, 0, 4);
        grid.add(extendedValue, 2, 4);
